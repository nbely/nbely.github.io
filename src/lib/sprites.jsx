...
export function TrainerSprite({ scale = 5, facing = 'down', idle = true }) {
  ...
  const W = 24; const H = 32;
  const c = SPRITE_COLORS;
  let _pxKey = 0;
  const px = (x, y, color, w = 1, h = 1) =>
    <rect key={`p${_pxKey++}`} x={x} y={y + bob} width={w} height={h} fill={color} shapeRendering="crispEdges" />;
  ...
  rects.push(px(9, 6, c.hairA)); rects.push(px(14, 6, c.hairA));
  rects.push(px(8, 8, c.skin)); rects.push(px(15, 8, c.skin));
  rects.push(px(10, 8, c.outline)); rects.push(px(13, 8, c.outline));
  ...
  for (let y = 12; y <= 18; y++) { rects.push(px(6, y, c.shirt)); rects.push(px(17, y, c.shirt)); }
  ...
  for (let y = 15; y <= 16; y++) for (let x = 11; x <= 14; x++) rects.push(px(x, y, c.laptopScreen));
  rects.push(px(9, 17, c.outline)); rects.push(px(15, 17, c.outline));
  rects.push(px(9, 18, c.outline)); rects.push(px(15, 18, c.outline));
...
export function Portrait({ speaking = false, scale = 3 }) {
  ...
  let _pxKey = 0;
  const px = (x, y, color, w = 1, h = 1) =>
    <rect key={`p${_pxKey++}`} x={x} y={y} width={w} height={h} fill={color} shapeRendering="crispEdges" />;
  const rects = [];
  ...
  rects.push(px(5, 4, c.hairA)); rects.push(px(10, 4, c.hairA));
  rects.push(px(4, 6, c.skin)); rects.push(px(11, 6, c.skin));
  rects.push(px(6, 7, c.outline)); rects.push(px(9, 7, c.outline));
  if (mouthOpen) {
  ...
  return (
    <div className="portrait" style={{ width: W * scale, height: H * scale }}>
      <svg width={W * scale} height={H * scale} viewBox={`0 0 ${W} ${H}`} style={{ imageRendering: 'pixelated' }}>
        {rects}
      </svg>
    </div>
  );
...
export function DexArt({ kind = 'generic', scale = 7 }) {
  const W = 16, H = 16;
  let _pxKey = 0;
  const px = (x, y, color, w = 1, h = 1) =>
    <rect key={`p${_pxKey++}`} x={x} y={y} width={w} height={h} fill={color} shapeRendering="crispEdges" />;
  ...
  if (kind === 'flowcord') {
    ...
    rects.push(px(2, 3, outline)); rects.push(px(13, 3, outline));
    rects.push(px(2, 10, outline)); rects.push(px(13, 10, outline));
    for (let x = 3; x <= 12; x++) { rects.push(px(x, 2, outline)); rects.push(px(x, 11, outline)); }
    for (let y = 4; y <= 9; y++) { rects.push(px(1, y, outline)); rects.push(px(14, y, outline)); }
    rects.push(px(4, 12, '#1fa463')); rects.push(px(5, 12, '#1fa463'));
    rects.push(px(4, 12, outline)); rects.push(px(3, 13, outline));
    ...
    for (let y = 4; y <= 10; y++) for (let x = 5; x <= 10; x++) {
      rects.push(px(x, y, y <= 6 ? '#c4463b' : '#f3ecd1'));
    }
    for (let x = 5; x <= 10; x++) { rects.push(px(x, 4, outline)); rects.push(px(x, 10, outline)); }
    for (let y = 4; y <= 10; y++) { rects.push(px(5, y, outline)); rects.push(px(10, y, outline)); }
    ...
    rects.push(px(11, 3, '#f2c230')); rects.push(px(13, 3, '#f2c230'));
    rects.push(px(12, 4, '#f2c230'));
  } else {
    rects.push(px(0, 0, '#f3ecd1', W, H));
    for (let x = 2; x <= 13; x++) { rects.push(px(x, 2, outline)); rects.push(px(x, 13, outline)); }
    for (let y = 2; y <= 13; y++) { rects.push(px(2, y, outline)); rects.push(px(13, y, outline)); }
    rects.push(px(7, 4, outline, 2, 1));
    rects.push(px(6, 5, outline)); rects.push(px(9, 5, outline));
    rects.push(px(9, 6, outline)); rects.push(px(8, 7, outline));
    rects.push(px(7, 8, outline, 2, 1));
    rects.push(px(7, 10, outline, 2, 1));
  }

  return (
    <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="xMidYMid meet" style={{ imageRendering: 'pixelated' }}>
      {rects}
    </svg>
  );
}

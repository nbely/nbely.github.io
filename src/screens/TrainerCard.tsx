...
const TRAINER_STATS: TrainerStat[] = [
...
const WORK_HISTORY: WorkEntry[] = [
  {
    ...
    period: '2023 — PRESENT',
    blurb:
      'ESG data platform for private markets. Shipped reporting, data ingestion, and internal tooling work.',
    roles: [
  ...
  },
  {
    ...
    period: '2020 — 2023',
    blurb:
      'Consulting agency. Built and shipped client products across multiple stacks — first dev role out of school.',
    roles: [
    ...
    <div className={`tc-root ${side === 'back' ? 'is-back' : ''}`}>
      <div className="tc-header">
        <div>
          ...
          <div className="tc-sub mono">
            {side === 'front' ? 'FRONT · stats & badges' : 'BACK · work history'}
          </div>
        </div>
        <button
          className="tc-flip"
                ...
                {WORK_HISTORY.map((job) => (
                  <article className="tc-job" key={job.company}>
                    ...
                    <ul className="tc-role-list">
                      {job.roles.map((r) => (
                        <li key={r.title}>
                          <span className="tc-role-bullet" aria-hidden="true">
                            ▸
                          </span>

...
import { DexArt } from '../lib/sprites';
...
const ENTRIES = [
  ...
  },
  {
    ...
    actions: [
      { label: 'GITHUB ↗', href: 'https://github.com/nbely/pokesandbox', primary: true },
    ],
    featured: true,
    art: 'pokesandbox',
    quest: {
  ...
  },
  {
    ...
    stats: [['ROLE', 'Solo'], ['STATUS', 'Archived']],
  ...
  },
  {
    ...
    stats: [['ROLE', 'Solo'], ['STATUS', 'Tinkering']],
    actions: [{ label: 'GITHUB ↗', href: '#' }],
    featured: false,
    art: 'generic',
  },
...
const QUEST_TREE = (quest) => ({
  root: { text: quest.lines[0], choices: [{ label: 'GO ON…', next: 'n1' }, { label: 'LATER', close: true }] },
...
export default function DevDex() {
  ...
  const onQuestClick = (entry) => {
    if (!entry.quest) return;
    if (!state.questsVisible) return;
    setActiveQuest(entry.quest);
    setQuestNode('root');
    completeQuest(entry.quest.id);
  };

  const questTree = activeQuest ? QUEST_TREE(activeQuest) : null;

  const handleQuestChoose = (choice) => {
  ...
  return (
    <div className="dex-root">
      <div className="dex-header">
        <div>
          <h2>DEV-DEX</h2>
          <div className="count">No. {String(filtered.length).padStart(3, '0')} / {String(ENTRIES.length).padStart(3, '0')} entries</div>
        </div>
        <div className="filter-select">
          <div className="select-wrap">
            <span className="select-prefix" aria-hidden="true">FILTER ·</span>
            <select
              id="dex-filter"
      ...
      <div className="dex-list">
        {filtered.map(entry => {
          ...
          return (
            <article
              key={entry.id}
              className={`dex-entry ${entry.featured ? 'featured' : ''}`}
            >
              {state.questsVisible && entry.quest && (
                <span
                  className={`marker quest-mk ${done ? 'done' : ''}`}
                  onClick={(e) => { e.stopPropagation(); onQuestClick(entry); }}
              ...
              <div className="dex-head">
                ...
                <div className="types">
                  {entry.types.map((t, i) => (
                    <span key={t} className={`type-chip ${i === 0 ? 'emerald' : i === 1 ? 'gold' : ''}`}>{t}</span>
                  ))}
                </div>
              </div>
              <div className="dex-row">
                ...
                <div className="dex-content">
                  ...
                  <div className="stats">
                    {entry.stats.map(([k, v]) => <span key={k}><b>{k}</b> &nbsp;{v}</span>)}
                  </div>
                  <div className="actions">
                    {entry.actions.map((a, i) => (
                      a.href
                        ? <a key={i} className={`btn ${a.primary ? 'primary' : ''}`} href={a.href} target="_blank" rel="noopener noreferrer">{a.label}</a>
                        : <button key={i} className={`btn ${a.primary ? 'primary' : ''}`}
      ...
      {(state.playMode || activeQuest) && (
        <div className="docked-dlg">
          <DialogueBox
            speaker="NICK"
            node={activeQuest ? questTree[questNode] : {
            ...
            onChoose={(c) => {
              if (activeQuest) { handleQuestChoose(c); return; }
              if (c.goto) { logDialogue({ who: 'YOU', text: c.label }); window.dispatchEvent(new CustomEvent('nav-go', { detail: c.goto })); }
